...
let cityName = document.querySelector("#city-search").addEventListener("click", getDataAboutCity)

function getDataAboutCity() {
  city = document.querySelector("#city-field").value; 
  apiUrl = `https://api.shecodes.io/weather/v1/current?query=${encodeURI(city)}&key=acfd102152eb04ebf39439at08oab84c&units=metric`;
  console.log(apiUrl);
  axios.get(apiUrl).then(displayTemperature);
}